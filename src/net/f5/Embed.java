...
//        String comment = "JPEG Encoder Copyright 1998, James R. Weeks and BioElectroMech.  ";
        ...
        for (i = 0; i < args.length; i++) {
            if (!args[i].startsWith("-")) {
                if (!haveInputImage) {
                    if (!args[i].endsWith(".jpg") && !args[i].endsWith(".JPG") && !args[i].endsWith(".tif") && !args[i].endsWith(".gif")
                            && !args[i].endsWith(".bmp")) {
                        StandardUsage();
                        return;
                    }
    ...
    public static void StandardUsage() {
        ...
        System.out.println("-p <password>\t\tdefault: \"zhenshanren\", only used when -e is specified");
